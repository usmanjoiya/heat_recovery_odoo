...
class QuickBookOnlineOrderFun(models.TransientModel):
    _inherit = 'quickbookonline.synchronization'

    def export_sync_order_data(self,connection, instance_id, to_export_id):
        ...
        for i in to_export_id:
            export_ids.append(self.env['sale.order'].browse(i))
        for i in export_ids:
